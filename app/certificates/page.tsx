...
import { TiltCard, CountUp, ScrollScale, FloatingOrbs } from "@/components/scroll/scroll-primitives";
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-8">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500/10 border border-green-500/30">
              ...
              <span className="font-mono text-sm text-green-400">
                <CountUp to={certificateImages.length} /> verified credentials
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {certificateImages.map((cert, index) => (
              <ScrollScale key={cert.id} from={0.92}>
              <TiltCard intensity={9} className="h-full">
              <motion.div
                initial={{ opacity: 0, y: 20, rotateX: -8 }}
                whileInView={{ opacity: 1, y: 0, rotateX: 0 }}
                transition={{ delay: (index % 6) * 0.08, duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
